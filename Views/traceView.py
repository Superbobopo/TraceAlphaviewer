...
class TraceView(BaseView):
    ...
    def _build_split_layout(self) -> None:
        ...
        self._split_pane.pack(fill='both', expand=True, side='top')
        self._split_pane.bind('<ButtonRelease-1>', self._remember_details_height)

        self._middle_pane = ctk.CTkFrame(self._split_pane, fg_color='#12121f', corner_radius=0)
        self._bottom_pane = ctk.CTkFrame(self._split_pane, fg_color='#0f0f1c', corner_radius=0)

        self._split_pane.add(self._middle_pane, minsize=CANVAS_H + 20)
        self._split_pane.add(self._bottom_pane, minsize=46)

        self._build_middle(self._middle_pane)
        self._build_bottom(self._bottom_pane)
        self.after(80, self._place_initial_split)

    def _place_initial_split(self) -> None:
        if not hasattr(self, '_split_pane') or self._split_ready:
            return

        height = self._split_pane.winfo_height()
        if height <= 1:
            self.after(80, self._place_initial_split)
            return

        top_min = min(CANVAS_H + 20, max(220, height - 46))
        bottom_target = min(self._details_height, max(46, height - top_min))
        sash_y = max(top_min, height - bottom_target)
        self._split_pane.sash_place(0, 0, sash_y)
        self._split_ready = True

    def _remember_details_height(self, _event=None) -> None:
    ...
    def _current_bottom_height(self) -> int:
        ...
        if height <= 1:
            return self._details_height
        try:
            _x, sash_y = self._split_pane.sash_coord(0)
        except tk.TclError:
            return self._details_height
        return max(46, height - sash_y)
    ...
    def _set_bottom_height(self, bottom_height: int) -> None:
        ...
        height = self._split_pane.winfo_height()
        if height <= 1:
            self.after(40, lambda: self._set_bottom_height(bottom_height))
            return

        top_min = min(CANVAS_H + 20, max(220, height - 46))
        max_bottom = max(46, height - top_min)
        target_bottom = min(max(46, bottom_height), max_bottom)
        self._split_pane.sash_place(0, 0, height - target_bottom)
    ...
    def _build_bottom(self, parent) -> None:
        ...
        ctk.CTkSlider(nav_bar, from_=0.1, to=10.0,
                      variable=self._speed_var, width=70, height=14,
    ...
    def _build_fallback_tabs(self, parent) -> None:
        ...
        for name in ('Diagnostic', 'Evenements', 'Trace'):
    ...
    def _on_speed_change(self, val) -> None:
        self._play_delay = int(PLAY_DELAY_DEFAULT / max(0.1, float(val)))
